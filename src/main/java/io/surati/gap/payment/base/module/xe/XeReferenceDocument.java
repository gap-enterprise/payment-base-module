		...
		super(
			new XeDirectives(
				...
				.add(name)
					...
					.add("abbreviated").set(item.beneficiary().abbreviated()).up()
					.add("beneficiary").set(item.beneficiary().name()).up()
					.add("code").set(item.beneficiary().code()).up()
					.add("beneficiary_id").set(item.beneficiary().id()).up()
					.add("advanced_amount").set(new FrWYSIWYGAmount(item.priorAmountPaid())).up()
				.up()
			)
		);
	}
